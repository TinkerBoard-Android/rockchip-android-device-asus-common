...
else ifeq ($(TARGET_PRODUCT), $(filter $(TARGET_PRODUCT),Tinker_Board_3))
	LOCAL_SRC_FILES := config/Tinker_Board_3/$(LOCAL_MODULE).apk
else
...
ifeq ($(TARGET_PRODUCT), $(filter $(TARGET_PRODUCT),WW_Tinker_Board_2 Tinker_Board_2))
	...
	LOCAL_SRC_FILES := config/Tinker_Board_3/$(LOCAL_MODULE)
else
...
ifeq ($(TARGET_PRODUCT),CN_Zenbo)
	...
	ifeq ($(TARGET_BUILD_VARIANT),user)
		LOCAL_SRC_FILES := config/Tinker_Board_3/user/$(LOCAL_MODULE)
	else
		LOCAL_SRC_FILES := config/Tinker_Board_3/debug/$(LOCAL_MODULE)
	endif
else ifeq ($(TARGET_PRODUCT), $(filter $(TARGET_PRODUCT),WW_Tinker_Board_2 Tinker_Board_2))
	ifeq ($(TARGET_BUILD_VARIANT),user)
		LOCAL_SRC_FILES := config/Tinker_Board_2/user/$(LOCAL_MODULE)
	else
		LOCAL_SRC_FILES := config/Tinker_Board_2/debug/$(LOCAL_MODULE)
	endif
else ifeq ($(TARGET_PRODUCT), $(filter $(TARGET_PRODUCT),WW_Tinker_Board Tinker_Board))
	ifeq ($(TARGET_BUILD_VARIANT),user)
		LOCAL_SRC_FILES := config/Tinker_Board/user/$(LOCAL_MODULE)
	else
		LOCAL_SRC_FILES := config/Tinker_Board/debug/$(LOCAL_MODULE)
