...
TARGET_AUTO_PQ_IMAGE=`get_build_var TARGET_AUTO_PQ_IMAGE`
...
echo TARGET_AUTO_PQ_IMAGE==$TARGET_AUTO_PQ_IMAGE
...
if [ "$TARGET_AUTO_PQ_IMAGE"x != ""x ]
then
    if [ -f $TARGET_AUTO_PQ_IMAGE ]
    then
        echo -n "create auto pq..."
        cp -a $TARGET_AUTO_PQ_IMAGE $IMAGE_PATH/autopq.img
        echo "done."
    else
        echo "$TARGET_AUTO_PQ_IMAGE not fount!"
    fi
fi
